...
// https://github.com/lightninglabs/taproot-assets/blob/main/taprpc/priceoraclerpc/price_oracle.proto
//
// It is reproduced here unmodified so that Tassandra can implement the
// PriceOracle service and act as a drop-in external price oracle for tapd.
...
//
//   - `F_c` is the coefficient component of the fixed-point representation. It
//     is the scaled-up fractional value represented as an integer.
//
...
// AssetSpecifier is a union type for specifying an asset by either its asset
// ID or group key.
...
type QueryAssetRatesRequest struct {
	...
	// metadata is an optional text field for additional transaction context.
	// Suggested format is a JSON string. Maximum length is 32,768 bytes.
	Metadata string `protobuf:"bytes,9,opt,name=metadata,proto3" json:"metadata,omitempty"`
	// node_id is the 33-byte public key of the local node making the request.
...
func (x *QueryAssetRatesRequest) GetNodeId() []byte {
	if x != nil {
		return x.NodeId
	}
	return nil
...
const file_priceoraclerpc_price_oracle_proto_rawDesc = "" +
	...
	"\x10asset_rates_hint\x18\x06 \x01(\v2\x1a.priceoraclerpc.AssetRatesR\x0eassetRatesHint\x12.\n" +
	"\x06intent\x18\a \x01(\x0e2\x16.priceoraclerpc.IntentR\x06intent\x12'\n" +
	"\x0fcounterparty_id\x18\b \x01(\fR\x0ecounterpartyId\x12\x1a\n" +
	"\bmetadata\x18\t \x01(\tR\bmetadata\x12\x17\n" +
	"\anode_id\x18\n" +
	" \x01(\fR\x06nodeId\"X\n" +
	...
	"\x04code\x18\x02 \x01(\x0e2\x19.priceoraclerpc.ErrorCodeR\x04code\"\xa4\x01\n" +
	...
	"\x1bINTENT_RECV_PAYMENT_QUALIFY\x10\x06*W\n" +
	"\tErrorCode\x12!\n" +
	"\x1dUNSPECIFIED_ORACLE_ERROR_CODE\x10\x00\x12'\n" +
	"#UNSUPPORTED_ASSET_ORACLE_ERROR_CODE\x10\x012q\n" +
	"\vPriceOracle\x12b\n" +
	"\x0fQueryAssetRates\x12&.priceoraclerpc.QueryAssetRatesRequest\x1a'.priceoraclerpc.QueryAssetRatesResponseB@Z>github.com/lightninglabs/tassandra/tassandrarpc/priceoraclerpcb\x06proto3"
